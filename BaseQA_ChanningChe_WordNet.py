...
if not os.path.exists(fname):  #Check if we already computed the best candidate sentences and thier entity tags
    ...
    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    POS_taggedQuestions= []
    for questionText in allQuestionText:
        questionList = ''.join(ch for ch in questionText if ch not in PunctuationExclude)
        questionList2 = [stemmer.stem(x.lower()) for x in questionList.split()]
        POS_taggedQuestions.append(questionList2)
    #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
def check_Synset(answer_word,question_word):
    for x in wn.synsets(answer_word,'n'):
        if x == wn.synsets(question_word,'n')[0]:
            return True
        if x in wn.synsets(question_word,'n')[0].hyponyms():
            return True
        if x in wn.synsets(question_word,'n')[0].part_meronyms():
            return True
    return False
#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

def extractAnswer(taggedQuestion,questionType,taggedBestAnswerSent,answerSentText,guessOTHERtype,apply3rdRule):
    answerList = []
    t = 0
    # trying to find questionType entity in answer
    #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    if questionType == 'OTHER':
        if taggedQuestion[0][0] == 'what':
            questionKey = None
            for que in taggedQuestion:
                if que[1] == 'NN':
                    questionKey = que[0]
                    break
            if questionKey is not None:
                for t in range(0, len(taggedBestAnswerSent) - 1):
                    guessedAnswerText = ""
                    if check_Synset(taggedBestAnswerSent[t][0],questionKey) is True:
                        for l in range(t, len(taggedBestAnswerSent) - 1):
                            if check_Synset(taggedBestAnswerSent[l][0],questionKey) is True:
...
for article in data:
    for question in article['qa']:  #For all questions in the article, but notice that we add all questions to the same list in the end, indexed by i
        ...
        #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        guessedAnswerText,filteredAnswers = extractAnswer(POS_taggedQuestions[i],questionType,taggedBestAnswerSent,answerSentText,False,True)
        ...
        if(guessedAnswerText =="" or guessedAnswerText == " " or guessedAnswerText.lower() in stopwordsAll):
            guessedAnswerText,filteredAnswers = extractAnswer(POS_taggedQuestions[i],questionType,taggedSecondBestAnswer,secondAnswerSentText,False,True)

        if (guessedAnswerText == "" or guessedAnswerText == " " or guessedAnswerText.lower() in stopwordsAll):
            guessedAnswerText,filteredAnswers = extractAnswer(POS_taggedQuestions[i],questionType, taggedBestAnswerSent, answerSentText, True, False)

        if (guessedAnswerText == "" or guessedAnswerText == " " or guessedAnswerText.lower() in stopwordsAll):
            guessedAnswerText,filteredAnswers = extractAnswer(POS_taggedQuestions[i],questionType, taggedSecondBestAnswer, secondAnswerSentText, True, False)
        if (guessedAnswerText == "" or guessedAnswerText == " " or guessedAnswerText.lower() in stopwordsAll):
            blank+=1
        #<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
