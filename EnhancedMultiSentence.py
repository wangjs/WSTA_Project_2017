...
def is_number(s): #A basic function to check if a word/token is a number or not
    try:
        ...
        # print(s)
        if s.lower() in wordNumbers or s.lower() in dateNumbers or checkIfRomanNumeral(s) or bool(re.match(ur'([£$€])(\d+(?:\.\d{2})?)', s)):
            return True
...
def extractAnswer(questionType,taggedBestAnswerSent,answerSentText,guessOTHERtype,apply3rdRule):
    ...
    if(guessOTHERtype):
        allQTypesList = ["NUMBER","PERSON","ORGANIZATION","LOCATION","OTHER"]
...
for article in data:
    for question in article['qa']:  #For all questions in the article, but notice that we add all questions to the same list in the end, indexed by i
        ...
        if(guessedAnswerText =="" or guessedAnswerText == " " or guessedAnswerText.lower() in stopwordsAll):
            guessedAnswerText,filteredAnswers = extractAnswer(questionType,taggedThirdBestAnswer,thirdAnswerSentText,False,False)
        if(guessedAnswerText =="" or guessedAnswerText == " " or guessedAnswerText.lower() in stopwordsAll):
            guessedAnswerText,filteredAnswers = extractAnswer(questionType,taggedThirdBestAnswer,thirdAnswerSentText,True,False)
        if (guessedAnswerText == "" or guessedAnswerText == " " or guessedAnswerText.lower() in stopwordsAll):
            blank+=1
